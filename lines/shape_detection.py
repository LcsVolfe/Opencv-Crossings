    ...
    if grid_detection and len(x_up) > 1 and len(x_down) > 1 and len(y_up) > 1 and len(y_down) > 1:
        ...
        for c in cnts:
            ...
            if gray_thresh[cY, cX] == 0:
                ...
                if cX > media_cX_1 and ultimoGridVisitado != 3:
                    ultimoGridVisitado=3
                    grids_visitados.append(3)
                        
                    contador = contador+1
                else:
                    ultimoGridVisitado=1  
                # # # 2 linha
                # elif ((cX > media_cX_0) or (cX > media_cX_1)) and ultimoGridVisitado != 2:
                #     ultimoGridVisitado=2
                #     grids_visitados.append(2)
                #     contador = contador+1
                    
                # # # 1 linha
                # elif cX < media_cX_0 and ultimoGridVisitado != 1:
                #     ultimoGridVisitado=1
                #     grids_visitados.append(1)
                #     contador = contador+1
            ...
            cv2.putText(frame_drawable, ("Array: " + str(len(grids_visitados))), (350, 50),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)   
